...
%   30/June/2004 debug
%   11/8/09 dlf try to free up some memory
...
fmx=zeros(nfr,nhmx);
vx=zeros(nfr,nhmx);
% fvi=zeros(nu-nl+1,fftl/2+1); %dlf
vfs=zeros(nfr,nhmx);
% avi=fvi*0; %dlf
lenx=length(x);
...
smmp=fftfilt((hanning(sml).^2)/sum((hanning(sml).^2)),[mmp zeros(fftl/2+1,sml*2)]'+0.00001)';%replaced with below for memory purposes 10/31/09 dlf
clear mmp %dlf
