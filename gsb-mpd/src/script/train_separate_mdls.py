...
Each classifier is a one layer NN logistic binary classifier.
...
from mpd.classifiers import L2ArcticFCnet, L2ArcticFCnetJoint,\
    L2ArcticFCnetJointReLU, L2ArcticFCnetJointDropout, L2ArcticFCnetJointBN,\
    L2ArcticFCnetJointBNv2, L2ArcticFCnetJointMultiLayer
from mpd.l2_aratic_mpd_dataset import L2ArcticMPDset
from sklearn.metrics import precision_recall_curve
from torch.nn.parameter import Parameter


def train_one_model(train_data, nn_model_type='L2ArcticFCnetJoint', rbm=None):
    if nn_model_type == 'L2ArcticFCnetJoint':
        ...
        model = L2ArcticFCnetJointBN(
            input_size, hidden_size, num_classes)
    elif nn_model_type == 'L2ArcticFCnetJointBNv2':
        model = L2ArcticFCnetJointBNv2(
            input_size, hidden_size, num_classes)
    ...
    if rbm is not None:
        ...
        model.fc1.bias = Parameter(rbm.hidden_bias)
        # The RBM trainer is input*output while the FC layer in Pytorch is
        # output*input
        model.fc1.weight = Parameter(rbm.weights.transpose(0, 1))
    ...
    for epoch in range(num_epochs):
        for i, (curr_feats, curr_labels) in enumerate(train_loader):
            ...
            curr_labels = \
                curr_labels[:, 0].type(torch.FloatTensor).view(-1, 1).to(device)
...
if __name__ == '__main__':
    ...
    for key, val in sym_table.items():
        ...
        if is_train:
            ...
            train_dataset = L2ArcticMPDset(train_files, sym_table,
                                           keep_phonemes={key},
                                           keep_annotations={'s'})
            ...
            mdl = train_one_model(train_loader, model_type, rbm)
        ...
        with torch.no_grad():
            ...
            for feats, labels in test_loader:
                ...
                for ii, phoneme_idx in enumerate(labels[:, 1]):
                    feat = feats[ii, :].view(-1, 1).transpose(0, 1)
                    mdl = model_dict[int(phoneme_idx.cpu())]
                    mdl.eval()
                    outputs[ii] = mdl(feat)[0].cpu().numpy()
                if predicts.size > 0:
